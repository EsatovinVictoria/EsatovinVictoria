<div align="center">

# Hi, I'm Esatovin Victoria 👋

### Informatics Engineering Student | Web Developer | Machine Learning Enthusiast | Back-end Enthusiasts 

<p>
  Curiosity always kills me...😫
</p>

<p>
  <a href="https://github.com/EsatovinVictoria">
    <img src="https://komarev.com/ghpvc/?username=EsatovinVictoria&label=Profile%20Views&style=for-the-badge" alt="Profile Views"/>
  </a>
</p>

</div>

---

## 👨‍💻 About Me

I'm an **Informatics Engineering student at Politeknik Negeri Malang** with an interest in software development, mobile applications, and artificial intelligence.

* 🎓 Studying **Informatics Engineering**
* 💻 Interested in **Web Development**
* 📱 Currently exploring **Flutter & Mobile App Development**
* 🤖 Learning **Machine Learning & Computer Vision**
* 🧠 Interested in applying AI to solve real-world problems
* 🚀 Always learning and experimenting with new technologies

---

## 🛠️ Tech Stack

### Languages

<p>
  <img src="https://skillicons.dev/icons?i=php,python,dart,java,js,html,css" />
</p>

### Frameworks & Technologies

<p>
  <img src="https://skillicons.dev/icons?i=laravel,flutter,tensorflow" />
</p>

### Database

<p>
  <img src="https://skillicons.dev/icons?i=mysql,postgresql" />
</p>

### Tools

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,figma,androidstudio" />
</p>

---

## 🚀 Featured Projects

### 🏥 Poliklinik Reservation System

A web-based clinic reservation system designed to simplify doctor scheduling and patient reservation management.

The system provides features for doctor schedules, reservations, daily quotas, medical records, reservation status tracking, and administration.

**Tech Stack**

`Laravel` • `Filament` • `MySQL` • `PHP`

---

### 🔐 Network & Cyber Security Laboratory Website

A laboratory profile and content management website for the **Network & Cyber Security Laboratory**.

The system provides public laboratory information and an administrative CMS for managing profiles, research archives, community service activities, galleries, agendas, facilities, and consultation messages.

**Tech Stack**

`PHP` • `PostgreSQL` • `JavaScript` • `HTML` • `CSS`

---

## 📊 GitHub Statistics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=EsatovinVictoria&show_icons=true&hide_border=true&theme=tokyonight" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=EsatovinVictoria&layout=compact&hide_border=true&theme=tokyonight" />

</div>

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=EsatovinVictoria&theme=tokyonight&hide_border=true" />

</div>

---

## 🌱 Currently Learning

<p align="center">

![Flutter](https://img.shields.io/badge/Flutter-Mobile%20Development-02569B?style=for-the-badge\&logo=flutter\&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-AI-FF6F00?style=for-the-badge\&logo=tensorflow\&logoColor=white)
![Computer Vision](https://img.shields.io/badge/Computer%20Vision-OpenCV-5C3EE8?style=for-the-badge\&logo=opencv\&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-Web%20Development-FF2D20?style=for-the-badge\&logo=laravel\&logoColor=white)

</p>

---

## 📫 Connect With Me

<div align="center">

<a href="https://github.com/EsatovinVictoria">
  <img src="https://img.shields.io/badge/GitHub-EsatovinVictoria-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://www.instagram.com/eastvinv/">
  <img src="https://img.shields.io/badge/Instagram-@eastvinv-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>

</div>

---

<div align="center">

### 💡 Obsession Beats Talent.

<sub>Come Back Anytime!✨</sub>

</div>
